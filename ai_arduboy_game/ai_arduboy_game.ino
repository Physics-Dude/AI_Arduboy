/*
  This is an Arduboy game titled: AI ARDUBOY (v1.1)
  ...
  1.1 - Moved 'hdd clicks' to "loadingIcon" so they tick in-game.
...
// Difficulty which defines character size and speeds
...
String loadSym = "/-\\|";
byte symState = 0;
bool loadingIcon(byte locx, byte locy) {
  ...
  if (!random(6)) {
    ...
    if (autoLoad == 64 || autoLoad == 100) return true;
    return false;
  }
}
...
void printerWrapper (String input) {
  ...
  // Simple but effective printing method saves a few bytes
  for (int i = 0; i < input.length(); i++) {
    arduboy.pollButtons();
    printer(input.charAt(i), letterWidth * i);
    if (arduboy.justPressed(UP_BUTTON)) quickUP = true; // if both are true, it prefers UP
    if (arduboy.justPressed(DOWN_BUTTON)) quickDOWN = true;
    if (arduboy.justPressed(B_BUTTON)) toggleSoundFun();
  }

  /*
    // Occasionally print a letter starting from the first instance of a space
    byte startPos = 0;
    byte i = 0;
    if (!random()) startPos = input.indexOf(' ');
    ...
    if (arduboy.justPressed(UP_BUTTON)) quickUP = true;
    if (arduboy.justPressed(DOWN_BUTTON)) quickDOWN = true;
    if (arduboy.justPressed(B_BUTTON)) toggleSoundFun();
    i++;
    startPos++;
...
void loop() {
  ...
  if (!gameStart) {
    ...
    while (!arduboy.pressed(A_BUTTON)) {
      if (arduboy.nextFrame()) {
        ...
        if (loadingIcon(123, 8)) drawSubText(1);
  ...
  if (gameStart) {
    ...
    do  {
      ...
      else if (arduboy.justPressed(DOWN_BUTTON) || quickDOWN) {
        if (soundON) beep.tone(1000);
        if (iWillBeConstructive) flashy(40);
        beep.noTone();
        choice = 0;
